...
const initialMessages: Message[] = [
  { id: 1, role: "assistant", content: "Hello! How can I assist you with your crypto-related questions today?" },
  { id: 2, role: "user", content: "What's the current balance of @Main Wallet?" },
  {
    id: 3,
    role: "assistant",
  ...
  { id: 4, role: "user", content: "Send 0.1 BTC from @Main Wallet to @Savings" },
  {
    id: 5,
    role: "assistant",
    content:
    ...
    instructions: [
      {
        ...
        description: "This step ensures that the source wallet has sufficient funds for the transfer.",
      },
      {
        action: "PREPARE_TRANSACTION",
        params: { fromWalletId: "@Main Wallet", toWalletId: "@Savings", amount: 0.1, currency: "BTC" },
        status: "in_progress",
        steps: [
      ...
      },
      {
        ...
        status: "idle",
        steps: [],
        description: "This step waits for user confirmation before proceeding with the transaction.",
      },
      {
        ...
        status: "idle",
        steps: [],
        description: "Once confirmed, this step sends the transaction to the blockchain network for processing.",
      },
      {
...
export function AIChat() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (lastAtSymbol !== -1) {
      ...
      setSuggestions(wallets.filter((wallet) => wallet.name.toLowerCase().includes(query)))
    } else {
      setSuggestions([])
    }
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (input.trim()) {
      setIsApproved(false)
      setIsDeclined(false)
      const newUserMessage: UserMessage = { id: messages.length + 1, role: "user", content: input.trim() }
      ...
      if (input.toLowerCase().includes("balance")) {
        newAssistantMessage = {
          ...
          role: "assistant",
          content: "I'll check the balance for you. Here are the steps I'm taking:",
          instructions: [
            {
              action: "FETCH_WALLET_BALANCE",
              description: "This step retrieves the current balance of the specified wallet from the blockchain.",
            ...
            },
            {
              ...
              description: "Once the balance is fetched, this step formats and displays the balance information.",
              params: { walletId: "@Main Wallet" },
              status: "idle",
              steps: [],
            },
      ...
      } else if (input.toLowerCase().includes("send") || input.toLowerCase().includes("transfer")) {
        newAssistantMessage = {
          id: messages.length + 2,
          role: "assistant",
          content:
          ...
          instructions: [
            ...
            },
            {
              action: "CONFIRM_TRANSACTION",
              description: "This step waits for user confirmation before proceeding with the transaction.",
              params: { transactionId: "0xabcdef1234567890abcdef1234567890abcdef12" },
              status: "idle",
              steps: [],
            },
            {
  ...
  const renderMessageContent = (message: Message) => {
                ...
                <div className="space-y-2">
                  {message.instructions.map((instruction, index) => (
                    <Accordion key={index} type="single" collapsible className="w-full">
                      <AccordionItem value={`instruction-${index}`} className="border rounded-lg">
                        <AccordionTrigger className="text-sm px-3 py-2 flex items-center">
                          <div className="mr-2">
                            {getStatusIcon(isApproved ? instruction.status : isDeclined ? "failed" : "idle")}
                          </div>
                          <span className="flex-grow text-left">{instruction.action}</span>
                        </AccordionTrigger>
                        <AccordionContent className="border-t">
                          <div className="text-sm p-3 space-y-4">
                            ...
                            <div className="mt-4">
                              ...
                              <div className="space-y-2">
                                {Object.entries(instruction.params).map(([key, value], index) => (
                            ...
                            <Card className="border rounded-lg">
                              <CardContent className="p-3">
                                ...
                                {instruction.steps.map((step, stepIndex) => (
                                  <div key={stepIndex} className="flex items-center space-x-2 mb-2 last:mb-0">
                                    <div>
                                      {getStatusIcon(isApproved ? step.status : isDeclined ? "failed" : "idle")}
                                    </div>
    ...
    return message.content.split(/(@\w+\s*\w*|0x[a-fA-F0-9]{40})/).map((part, index) => {
      if (part.startsWith("@")) {
        const walletName = part.slice(1).trim()
        if (wallets.some((wallet) => wallet.name === walletName)) {
          return (
        ...
        return (
          <span key={index} className="text-teal-300 inline-flex items-center">
            {truncatedAddress}
            <button
          ...
          <div className="space-y-4">
            {messages.map((m) => (
              <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className="rounded-lg p-3 max-w-[80%] bg-muted">{renderMessageContent(m)}</div>
              </div>
            ))}
          </div>
        </ScrollArea>
