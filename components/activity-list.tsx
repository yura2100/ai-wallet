...
export function ActivityList({ transactions, getBlockExplorerUrl, isLoading }: ActivityListProps) {
  return (
        ...
        {isLoading
          ...
          : transactions.map((tx) => (
              <li
                ...
                className="grid grid-cols-[1fr,0.5fr,2fr,2fr,auto] gap-2 items-center p-3 rounded-lg border h-[52px] transition-colors duration-200 ease-in-out hover:bg-muted"
              >
                <span className={`font-medium ${tx.type === "Received" ? "text-green-500" : "text-red-500"}`}>
                  {tx.type}
                </span>
                <span></span>
